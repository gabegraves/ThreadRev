...
| Web console | Soham (28gugales-dev) | 1:05 `8e6ee54` on branch `web-console`: Civic-styled console in `apps/web` — left rail, 8 pages (overview, thread, documents, findings, evidence graph, checker runs, analytics, chat), demo evidence logs generated from fixtures + `evals/records/scripted` incl. cross-channel A and the `workspace_search` event; web typecheck passes; preview :3110 | Workspace (exact-match index) page, force-layout evidence map; then PR to `main` | Gabe: review/merge `web-console` when ready |
