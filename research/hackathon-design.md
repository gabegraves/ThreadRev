...
6. **105-120 s. Rev asks before it writes.** The "Proposed edit: needs approval" card for section 4: `t = 6.91 s` → `t = 6.493 s`, checker run id, Approve and Reject. Click Approve. The card redraws: applied, new file, new hash, original untouched. Spoken: "It never writes on its own. It proposes the exact line, you approve, it writes a new copy and keeps the hash of the one you signed." If time is short, drop the replay scorecard, not this.
