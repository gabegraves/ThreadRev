...
2. **15-30 s. The ask.** Juno Marsh, who joined in August: "@reviewer can you check section 3 of the r2 doc before I sign the review?"
...
5. **85-105 s. What the surface gave it.** Spoken: a chatbox handed the same doc would have checked it against itself and passed it. The 680 came from Dara's message, the 2.5 s from Tam's, the 820 from a correction no document holds yet. If time allows, the cross-channel variant: the same correction posted two weeks earlier in `#ks4-purchasing` as a snubber order, found by `search_workspace` on the unit `uF`.
