...
import { DAY_MS, HOUR_MS } from "@/civic/lib/time-constants";

// Real time-to-supersession: finding_published.at -> finding_superseded.at.
// Closed reports without a recorded completed_at are skipped by every
// duration metric rather than given a synthetic value.
function resolutionHours(r: DashboardReport): number | null {
  if (r.status !== "closed" || !r.completed_at) return null;
  return Math.max(
    0,
    (Date.parse(r.completed_at) - Date.parse(r.created_at)) / HOUR_MS,
  );
}

function closeTime(r: DashboardReport): number | null {
  if (r.status !== "closed" || !r.completed_at) return null;
  return Date.parse(r.completed_at);
}

function closedWithHours(reports: DashboardReport[]): number[] {
  const out: number[] = [];
...
function mttr(reports: DashboardReport[]): number {
  const hours = closedWithHours(reports);
  if (!hours.length) return 0;
  return hours.reduce((s, h) => s + h, 0) / hours.length;
}
...
function slaCompliance(reports: DashboardReport[]): number {
  const closed = reports.filter((r) => resolutionHours(r) !== null);
  if (!closed.length) return 0;
  const met = closed.filter(
    (r) => (resolutionHours(r) ?? 0) <= categorySlaHours(r.category),
  ).length;
...
const FUNNEL_ORDER: ReadonlyArray<{ status: ReportStatus; label: string }> = [
  { status: "open", label: "Live" },
  { status: "dispatched", label: "Dispatched" },
  { status: "in_progress", label: "In progress" },
  { status: "closed", label: "Superseded" },
];
...
export function deriveCategoryResolution(
  ...
  for (const r of reports) {
    ...
    entry.count++;
    const h = resolutionHours(r);
    if (h !== null) entry.closedHours.push(h);
