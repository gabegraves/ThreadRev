...
import {
  ...
  Tile,
  Toggle,
} from "@/civic/analytics/bento-primitives";
import {
  TipBar,
  TipChip,
  TipRow,
  useHoverTip,
} from "@/civic/analytics/hover-tip";
import type {
  AnalyticsKpis,
  CategoryResolution,
  HeatCell,
  ...
  StatusFunnelStep,
  TrendPoint,
} from "@/civic/lib/analytics-data";
import type { SlaRisk } from "@/civic/filters/derive";
import { SEVERITY_HUE } from "@/civic/lib/severity-colors";
import { cn } from "@/civic-ui/lib/cn";
import { formatHours } from "@/civic/lib/time-ago";
          ...
          return {
            ...
            body: (
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[11px] text-faint leading-snug mt-1">
                  Share of superseded findings replaced inside their SLA window.
                </p>
              </div>
            ),
            footer: (
...
function renderTrendChart(
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Findings published vs superseded over time"
    >
      <defs>
...
function ReportsTrendInner({ data }: ReportsTrendProps) {
  ...
  if (data.length === 0) {
    return (
      <Tile title="Findings over time" className="lg:col-span-8 lg:row-span-2">
        <EmptyState message="No findings in range" />
      </Tile>
    );
  }
  ...
  const legendTip = (series: "created" | "closed") => {
    ...
    return {
      title: isCreated ? "Published" : "Superseded",
      accent: color,
      body: (
        <div className="flex flex-col gap-1.5">
          <p className="text-[11px] text-faint leading-snug">
            {isCreated
              ? "Finding cards published by the reviewer on each day."
              : "Cards marked stale by a newer card on each day."}
          </p>
  ...
  return (
    <>
      ...
      <ExpandModal
        ...
        title="Findings over time"
        subtitle={`${slice.length}d window · ${sliceClosed}/${sliceCreated} superseded`}
        chart={
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-4 text-[12px]">
              <Legend
                color="var(--pastel-sky-strong)"
                label={`Published · ${sliceCreated}`}
              />
              <Legend
                color="var(--pastel-mint-strong)"
                label={`Superseded · ${sliceClosed}`}
              />
            </div>
            <TrendChart
              data={slice}
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Daily count of findings published (primary) vs. superseded (green). The
              ratio of closed-to-created is the operational throughput — a ratio
              above 100% means newer cards are retiring old ones faster than they appear.
            </Prose>
            <StatGrid>
              <Stat
                label="Window"
  ...
  return [
    {
      ...
      hint:
        netBacklog > 0
          ? "more published than superseded"
          : netBacklog < 0
    ...
    },
    {
      ...
      hint: `${peak.created.toLocaleString()} published`,
      tone: "neutral",
      icon: "peak",
    },
    {
      ...
      hint:
        momentumPct >= 0
          ? "publishing rising vs prior"
          : "publishing easing vs prior",
      tone: "neutral",
...
const SEVERITY_DESC: Record<1 | 2 | 3 | 4 | 5, string> = {
  1: "1 failing check — or a clean run with nothing failing",
  2: "2 failing checks",
  3: "3 failing checks",
  4: "4 failing checks",
  5: "5 failing checks — every check the checker ran failed",
};
...
function renderDonut(
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Failing checks distribution donut chart"
    >
      <circle
        cx={size / 2}
        cy={size / 2}
      ...
      {data.map((s) => {
        ...
        const el = (
          <circle
            ...
            aria-label={
              bind
                ? `${s.severity} failing, ${s.count} findings, ${(frac * 100) | 0}%`
                : undefined
            }
            style={{
...
function SeverityDonutInner({ data }: SeverityDonutProps) {
  ...
  const tipFor = (s: SeveritySlice) => {
    ...
    return {
      ...
      body: (
        <div className="flex flex-col gap-1.5">
          <TipRow label="Findings" value={s.count.toLocaleString()} />
    ...
    <>
              ...
              {data.map((s) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span className="text-subtle">{s.severity} failing</span>
      ...
      <ExpandModal
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Failing checks are counted from the checker run each finding
              was published from (1-5 scale; a clean run counts as 1). More
              failing checks means more of the document disagreed with the
              recomputed numbers.
            </Prose>
            <StatGrid>
              <Stat label="Total" value={total.toLocaleString()} />
              <Stat
                label="Dominant count"
                value={`${dominant.severity} failing`}
                hint={`${dominant.count} findings`}
              />
              <Stat
                label="All-fail share"
                value={`${emergencyShare.toFixed(1)}%`}
                hint="5 failing checks"
              />
              <Stat
                label="Critical+"
...
> = {
  open: { label: "Live, unanswered", tone: "warn" },
  dispatched: { label: "Dispatched", tone: "neutral" },
  in_progress: { label: "Awaiting a decision", tone: "neutral" },
  closed: { label: "Superseded by a newer card", tone: "good" },
};
  ...
  return (
    <>
      ...
      <ExpandModal
        ...
        onClose={() => setOpen(false)}
        title="Workflow pipeline"
        subtitle={`${total} findings across all stages`}
        chart={
          <div className="py-4">
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Findings flow through four stages: a card goes live (Live) →
              it is dispatched to the thread (Dispatched) → a person must
              decide (In progress) → a newer card replaces it (Superseded).
              Drop-off between stages shows where review stalls.
            </Prose>
            <StatGrid>
              <Stat
                label="In flight"
                value={stuck.toLocaleString()}
                hint="Not yet superseded"
              />
              <Stat label="Superseded" value={closed.toLocaleString()} />
              <Stat
                label="Supersession rate"
                value={`${total ? ((closed / total) * 100).toFixed(0) : 0}%`}
              />
              <Stat
                label="Live, unanswered"
                value={(
      ...
      {data.map((b, i) => {
        ...
        return (
          ...
          <div
            ...
            aria-label={
              bind
                ? `${b.label}: ${b.count} findings, cumulative ${cum}%`
                : undefined
            }
            className={cn(
...
function ResolutionHistogramInner({ data }: ResolutionHistogramProps) {
  ...
  const bucketMeaning = (
    ...
    if (b.hours_max <= 168) {
      return { text: "Same-week supersession", tone: "warn", chip: "Slow" };
    }
    return {
  ...
  const tipForBar = (i: number) => {
    ...
    return {
      ...
      body: (
        <div className="flex flex-col gap-1.5">
          <TipRow label="Findings" value={b.count.toLocaleString()} />
          <TipRow label="Share" value={`${pct.toFixed(1)}%`} />
    ...
    <>
      ...
      >
        {total > 0 ? (
          ...
          <EmptyState message="No superseded findings in range" />
        )}
        <tip.Portal />
      </Tile>
      <ExpandModal
        open={open}
        onClose={() => setOpen(false)}
        title="Supersession time distribution"
        subtitle={`${total} superseded findings`}
        chart={
          <div className="py-6">
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Time from a card going live to a newer card replacing it,
              bucketed. A left-skewed (fast) curve means revisions land and
              get re-reviewed quickly.
            </Prose>
            <StatGrid>
              <Stat
                label="Within 24h"
                value={`${total ? ((within24h / total) * 100).toFixed(0) : 0}%`}
                hint={`${within24h} findings`}
              />
              <Stat
                label="Within 72h"
              ...
              {grid[d].map((cell) => {
                ...
                return (
                  <div
                    ...
                    aria-label={`${DAY_LABELS[cell.day]} ${String(cell.hour).padStart(2, "0")}:00, ${cell.count} findings`}
                    role="img"
                    {...(cellBind ?? {})}
                  />
                );
...
function PeakHoursHeatmapInner({ data }: PeakHoursHeatmapProps) {
  ...
  const tipForCell = (cell: HeatCell) => {
    ...
    return {
      ...
      body: (
        <div className="flex flex-col gap-1.5">
          <TipRow label="Findings" value={cell.count.toLocaleString()} />
  ...
  const tipForDay = (day: number) => {
    ...
    return {
      ...
      body: (
        <div className="flex flex-col gap-1.5">
          <TipRow label="Findings" value={dayTotal.toLocaleString()} />
    ...
    <>
      ...
      <ExpandModal
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              When cards are published. Density follows when engineers post
              revisions to the thread, so clusters mark the hours a design
              review is actually moving.
            </Prose>
            <StatGrid>
              <Stat
                label="Peak slot"
                value={`${DAY_LABELS[peak.day]} ${String(peak.hour).padStart(2, "0")}:00`}
                hint={`${peak.count} findings`}
              />
...
function TopNeighborhoodsInner({ data }: TopNeighborhoodsProps) {
  ...
  const tipFor = (n: NeighborhoodVolume, rank: number) => {
    ...
    return {
      ...
      body: (
        <div className="flex flex-col gap-1.5">
          <TipRow label="Total findings" value={n.count.toLocaleString()} />
          <TipRow
            label="Open"
          ...
          <TipRow label="Share of corpus" value={`${share.toFixed(1)}%`} muted />
  ...
  return (
    <>
      ...
      <ExpandModal
        ...
        subtitle={`${data.length} sources · ${total} findings`}
        chart={
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Concentration of findings by primary evidence source (document
              revision or message). Compare &ldquo;total&rdquo; vs.
              &ldquo;open&rdquo; ranking to find sources whose live cards are
      ...
      {rows.map((row) => {
        ...
        return (
          <li
            ...
            aria-label={
              bind
                ? `${row.label}, avg ${formatHours(row.avg_hours)}, target ${formatHours(row.target_hours)}, ${row.count} findings`
                : undefined
            }
            {...(bind ?? {})}
          >
...
function CategoryResolutionTableInner({ data }: CategoryResolutionTableProps) {
  ...
  return (
    <>
      <Tile
        title="Supersession time by category"
      ...
      <ExpandModal
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Each bar shows mean time to supersession per category; the
              vertical mark is the target window. Red bars exceed the window
              — cards in these categories stay stale-but-live longest.
            </Prose>
            <StatGrid>
              <Stat
                label="Over SLA"
              ...
              <Stat
                ...
                hint="Volume-weighted mean"
              />
              <Stat label="Findings" value={total.toLocaleString()} />
            </StatGrid>
          </div>
        }
      />
...
function ReporterVelocityCardInner({ data }: ReporterVelocityCardProps) {
  ...
  const bigNumberTip = (): Parameters<typeof tip.show>[0] => ({
    ...
    body: (
      <div className="flex flex-col gap-1.5">
        ...
        <p className="text-[11px] text-faint leading-snug mt-1">
          Review-load signal — breadth (unique engineers) vs depth (findings
          per engineer).
        </p>
      </div>
    ),
  ...
  const sparkTipFor = (
    ...
    return {
      ...
      body: (
        <div className="flex flex-col gap-1.5">
          <TipRow label="Engineers" value={v.toLocaleString()} />
  ...
  return (
    <>
        ...
        <div className="flex items-end justify-between gap-4">
          ...
          >
            ...
            <p className="text-[13px] text-subtle mt-2 inline-flex items-center gap-1.5">
              ...
              unique · {data.reports_per_reporter.toFixed(2)}/engineer
            </p>
          </div>
      ...
      <ExpandModal
        ...
        onClose={() => setOpen(false)}
        title="Engineer activity"
        subtitle={`${data.unique_reporters} unique engineers · ${data.spark.length}d`}
        chart={
          <div className="flex flex-col gap-6 py-2">
            <div className="flex flex-wrap items-end justify-between gap-6">
              <div>
                ...
                <p className="text-[13px] text-subtle mt-2 inline-flex items-center gap-1.5">
                  <Users className="h-3.5 w-3.5" strokeWidth={1.75} />
                  unique engineers · {data.reports_per_reporter.toFixed(2)}{" "}
                  findings each
                </p>
              </div>
              <div
        ...
        info={
          <div className="flex flex-col gap-5">
            <Prose>
              Review-load signal. Unique engineers indicates how many people
              triggered a review; findings-per-engineer indicates depth (does
              one thread keep producing cards, or is the load spread?).
            </Prose>
            <StatGrid>
              <Stat
                label="Unique engineers"
...
function RecurringHotspotsCardInner({ data }: RecurringHotspotsProps) {
  ...
  return (
    ...
    >
      {top.length === 0 ? (
        <EmptyState message="No recurring sources yet" />
      ) : (
          ...
          {top.map((h) => (
            ...
            >
              <div className="flex items-center justify-between gap-2 text-[13px]">
                ...
                <span className="tabular-nums text-faint flex-shrink-0">
                  {h.episodes}× weeks · {h.total} findings
                </span>
              </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {data.breached > 0 && (
            <p className="text-[12px] text-[var(--color-danger)]">
              {data.breached} finding{data.breached === 1 ? "" : "s"} past their
