...
import { downstreamOf, type EvidenceGraph, type GraphNode } from "agent-core/shared";
...
export function EvidenceGraphSvg({ graph, compact = false }: { graph: EvidenceGraph; compact?: boolean }) {
  ...
  const placed = useMemo(() => {
    ...
    return { nodes: out, columns, height: PAD * 2 + 18 + maxRows * ROW_H };
  }, [graph]);

  const highlighted = useMemo(() => {
    if (!hover) return null;
    const set = new Set(downstreamOf(graph, hover).nodes);
    set.add(hover);
    return set;
  }, [hover, graph]);

  const width = PAD * 2 + placed.columns.length * COL_W;
  ...
  return (
    <div className={`ck-tr-graph${compact ? " ck-tr-graph--compact" : ""}`}>
      <svg
        viewBox={`0 0 ${width} ${placed.height}`}
        width={compact ? "100%" : width}
        height={compact ? undefined : placed.height}
        role="img"
        aria-label="Evidence graph"
      >
        {!compact &&
          placed.columns.map((kind, col) => (
