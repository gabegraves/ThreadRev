      ...
      {rows.map((r, i) => (
        <li key={r.finding.finding_id} className={cn(i > 0 && "border-t border-hairline")}>
          <Link
            ...
            className="-mx-2 flex items-center gap-3 rounded-md px-2 py-2 transition-colors hover:bg-overlay focus-visible:outline-2 focus-visible:outline-accent"
          >
            <StatusPill tone={STATUS_TONE[r.tone]} className="shrink-0">
              {STATUS_LABEL[r.tone]}
            </StatusPill>
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[13px] text-foreground">{r.finding.discrepancy === "none" ? headline(r.finding) : r.finding.discrepancy}</span>
              <span className="block truncate font-mono text-[11px] text-faint">{r.finding.finding_id}</span>
            </span>
