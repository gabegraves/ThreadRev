...
[Judging criteria](#how-threadrev-meets-the-judging-criteria) · [Why not a chatbox](#why-this-cannot-be-a-chatbox) · [What it does](#what-it-does) · [Demo](#the-demo-scenario-a) · [How it works](#how-it-works) · [Failure handling](#failure-handling-and-trust-guarantees) · [Approval workflow](#approval-workflow-rev-proposes-a-person-approves-rev-writes-a-copy) · [Versus Slack AI](#what-slack-already-does-and-what-threadrev-adds) · [Why search, not RAG](#why-search-by-identifier-not-rag) · [Status](#status) · [Run it](#run-it) · [Sponsors](#sponsor-technologies) · [Built vs inherited](#what-we-built-and-what-we-inherited)
...
**Demo video (2 min):** _link added at submission_
...
2. **Bind to a revision.** Every card names the document revision, its SHA-256, and the Slack message timestamp of the last requirement change it was computed against.
...
|---|---|---|
| **Core Requirements & Functionality** — _robust, reliable, and fully functional within its intended environment_ | Card 1 posted live in Slack (19:22:30Z, 40 s from mention). The 19:03Z failure (revision-string mismatch) was diagnosed and fixed mid-day; see [Status](#status) for the full account. Offline: replay harness runs the real tools through every scenario and asserts the card. | 3:22 entry in [`STATUS.md`](STATUS.md); [`apps/channel/src/replay/`](apps/channel/src/replay/); [`evals/records/`](evals/records/) |
| **Innovation & Theme Alignment** — _a surprising new agent pattern whose central value could not be reproduced in a standalone chatbox_ | The thread revision clock, the cross-channel correction, editing the card to STALE in place, and the Approve button in the thread all require the Slack surface. A chatbox that received the same pasted text would repeat the printed number, find no contradiction, and have no card to mark stale. See [Why this cannot be a chatbox](#why-this-cannot-be-a-chatbox). | [`apps/channel/src/revision.ts`](apps/channel/src/revision.ts); [`workspace.ts`](apps/channel/src/workspace.ts); [`finding-card.tsx`](apps/channel/src/finding-card.tsx) |
| **Technical Execution & Integration** — _robust orchestration, thoughtful failure handling, and a deeply integrated architecture_ | Six-tool reviewer agent; stdlib Python checkers with JSON I/O; fail-closed freshness guard in `publish_result`; stale-in-place card update; approve-before-write gate in `propose_edit` with three hard refusal rules; append-only evidence log; graph query for downstream invalidation; replay harness; silence filter. The 19:03Z failure is a concrete example: diagnosed, root-caused, and fixed in one commit. | [`reviewer-tools.tsx`](apps/channel/src/reviewer-tools.tsx); [`checkers/`](checkers/); [`packages/agent-core/src/evidence/`](packages/agent-core/src/evidence/); [Failure handling](#failure-handling-and-trust-guarantees) |
| **Usefulness & Agentic Experience** — _native to its environment, using context intelligently while remaining clear and controllable_ | Card in the thread where the question was asked. Exact discrepancy, engineering consequence, source hashes, checker run id, who to ask. STALE instead of deleted. Approve/Reject in the thread, not in a separate UI. Silence on normal chatter. A desktop companion (Rev overlay above the Slack app) turns red while a finding is unseen and goes idle when the panel is opened. | [`finding-card.tsx`](apps/channel/src/finding-card.tsx); [`apps/pet/`](apps/pet/); [approval flow](#approval-workflow-rev-proposes-a-person-approves-rev-writes-a-copy); silence filter in [`review-moment.ts`](apps/channel/src/review-moment.ts) |

---

## Why this cannot be a chatbox

The "Agents, Everywhere" theme is: what becomes possible when the agent shows up where the work is already happening? For ThreadRev, five things require the Slack surface and disappear without it.

1. **The thread revision clock.** The revision that a card is bound to is the `ts` of the last requirement-change message in the thread — a Slack-native timestamp. Without the thread, there is no sequence, no revision, and no stale marker.
2. **The cross-channel correction.** Dara's 820 uF correction was posted in `#ks4-purchasing`, not `#ks4-electrical`. `search_workspace` finds it by searching the whole workspace with a cutoff at the trigger. A chatbox only sees what you paste; pasting the thread loses the other channel.
3. **Editing the card to STALE in place.** The old finding card is a live message in the thread. When Dara posts her correction, the bot edits that message — so the thread shows the full history, with the superseded finding visibly marked, not replaced. A chatbox reply is immutable; a new response does not invalidate the old one.
4. **Approve buttons in the thread.** The `propose_edit` card posts Block Kit buttons in the same thread as the finding. A person clicking Approve in that thread is approving that specific finding, in context, alongside the evidence. A chatbox cannot post a persistent button into the channel that produced the document.
5. **Silence on normal chatter.** Rev reads every message in the channel but posts only when the review gate fires. A chatbox speaks when prompted; Rev's value depends on speaking unprompted when it matters and staying silent when it does not — which requires being in the channel, not waiting to be addressed.
...
ThreadRev lives in the channel. When someone asks it to check a document, or when a message changes an input that an earlier finding depended on, it reads the thread, searches every other channel for the document and the values it rests on, recomputes the numbers with a local checker, and posts one card with the discrepancy, the engineering consequence, the sources with hashes and revisions, which numbers were reproduced versus inferred, and who can resolve it.

When a later message changes a requirement, the earlier card is marked **stale in place** and a new card bound to the new revision is posted. If a revision lands while a check is running, the result is refused before it posts. When a printed result does not reproduce and its inputs are not in dispute, Rev proposes the exact edit and waits for a person to click Approve.

Two rules shape everything:

- **The model never computes a number that appears on a card.** A local, stdlib-only Python checker does. `publish_result` is the only path to a card and copies numbers from the checker's response.
...
Kestrel Motors is a fictional maker of light electric city vehicles. The KS-4 is its next platform. Dara Voss is the electrical lead. Tam Holloway owns firmware, including the relay timer. Juno Marsh joined in August and has to sign the r2 design review. If the relay closes before the bus is charged, every key-on puts the inrush through the main contactor, so the number in the signed review is the number that ships. Channel `#ks4-electrical`, precharge RC timing. Fixture script: [`fixtures/slack/scenario-a.json`](fixtures/slack/scenario-a.json), full spec: [`research/synthetic-fixture-spec.md`](research/synthetic-fixture-spec.md).
...
4. **Card 1.** Section 3 text says 680 uF, section 2 diagram says 750 uF, and the printed 2.435 s only reproduces with 750 uF. At 680 uF it is 2.208 s. Section 4 worked example prints 6.91 s; recomputed, 6.493 s. Question to Dara: which capacitance is right. Card content: [`contracts/examples/finding-scenario-a.json`](contracts/examples/finding-scenario-a.json).
5. Dara: "Bus is 820 uF, not 680. Doc will be r3."
6. **Card 1 goes stale. Card 2.** At 820 uF, t\_99.9 = 2.662 s, later than the 2.5 s relay timer. The bus reaches 99.85 percent at 2.5 s. The timer or the resistor must change. Bound to Dara's message, because r3 does not exist yet. [`finding-scenario-a-superseding.json`](contracts/examples/finding-scenario-a-superseding.json).

**Scenario A, cross-channel** ([`scenario-a-cross.json`](fixtures/slack/scenario-a-cross.json)). Same thread, but Dara's 820 uF correction was posted two weeks earlier in `#ks4-purchasing`, as an order for a snubber bank. The thread alone cannot catch this. The reviewer calls `search_workspace` with unit `uF`, gets every capacitance message in the workspace up to the trigger, oldest first, and the card cites the purchasing message by channel and timestamp. Recorded run: [`evals/records/scripted/scenario-a-cross.1.json`](evals/records/scripted/scenario-a-cross.1.json).
...
- **Evidence log and graph** in [`packages/agent-core/src/evidence/`](packages/agent-core/src/evidence/). Every message read, document hashed, checker run, card published, refused publish, and silence decision is appended as an event. The graph builder turns the log into message, document, run, finding, and revision nodes with a downstream walk, so "what did this change invalidate" is a query.
- **Replay harness** in [`apps/channel/src/replay/`](apps/channel/src/replay/). Replays a fixture Slack script through the real channel handlers and real checkers offline, with a scripted agent standing in for the model, and asserts the card that gets posted.
- **Web review console** in [`apps/web/src/components/review-console/`](apps/web/src/components/review-console/), served by [`/api/evidence`](apps/web/src/app/api/evidence/route.ts). Live at [threadrev-web.vercel.app](https://threadrev-web.vercel.app). A browser view of the thread, the cards, and the evidence graph, polling the live log and falling back to the Scenario A sample.
- **Desktop companion** in [`apps/pet/`](apps/pet/). Rev is a frameless, always-on-top 72 px overlay that sits above the Slack desktop app. It polls `/api/pet/findings` from the web server: glyph is red and the character moves while a live finding is unseen; yellow while a stale finding is the newest; green otherwise. Opening the panel stops the motion. Pass `--endpoint=<url>` to point it at any running web server.
...
`publish_result` reads the thread's current revision before accepting a result. If the revision recorded at the start of the run does not match the revision now, it refuses to post the card. The refused result is kept as a stale record in the evidence log. This happened live at 19:03Z on September 12, 2026: the model passed a revision string derived from a fixture seed message it had seen through `search_workspace`, but the live thread's revision was the seed's `occurredAt` rather than the fixture timestamp, so `publish_result` refused as stale and the re-run exhausted the 10-step budget. Root cause identified in the evidence log; fixed in commit `21b6295` (`run_check` now records the thread revision at run time, so cards bind to the value that was current when the checker ran). Card 1 posted successfully at 19:22:30Z on the second attempt.

### Stale marking, not deletion
When a requirement-change message arrives after a card has been posted, the card is edited to **STALE** in place. The original finding, sources, and checker run id remain visible. A new card bound to the new revision is posted. No finding disappears; the thread shows the full history.

### Approve-before-write
`propose_edit` posts an Approve/Reject card and stops. The write happens only inside the button handler, when a person clicks Approve. Three hard rules are checked at call time before the card posts: the replacement value must be a number from the checker's output, the text to replace must contain a number, and it must occur exactly once in the document. A design choice, an input value, or an ambiguous case becomes the question on the finding card, not an edit proposal. The original file is never opened for writing; `apply_docx_edit.py` writes a new copy.

### Checker isolation
`check_rc.py` and `check_route.py` are stdlib-only. No network calls, no file writes, no imports outside the standard library. They receive a JSON object on stdin and return a JSON object on stdout. The model sees the output; it never computes arithmetic. Run ids on every card let an engineer inspect the exact inputs and outputs that produced the result.

### Silence filter
The reviewer reads every message in the channel but posts only when the review gate fires in `review-moment.ts`. It stays silent on normal chatter, posts a clean card when everything reproduces, and asks instead of deciding when sources conflict. This is an architectural property, not a prompt instruction.
...
Three events land in the evidence log — `edit_proposed`, `edit_decided`, `edit_applied` — each carrying the proposal id, the run id, and both hashes. The web console timeline shows them in order.

What cannot happen: a write without a click, a number the checker did not produce, a change to an input value or a design choice, a modified original, or a second decision on the same proposal. In Scenario A the section 3 timing is not proposed because its capacitance is the open question to Dara; only the fixed 2 mF worked example qualifies.
...
1. **A ranking can drop the one message that matters.** The correction that invalidates a document is a short message, often in the wrong channel, that says "820, not 680." Nothing about it is semantically close to "check section 3 of the r2 doc." ThreadRev searches by the identifiers an engineer would actually grep for: the document name, the unit, the words that name the quantity, the author. It returns every match up to the trigger, so the correction is in the set or it is not in the workspace. There is no similarity score to lose it to. There is a result cap, and a search that hits it says so — an unannounced cap would lose a correction exactly the way a ranking does.
...
The workspace index is retrieval. It is retrieval by exact match over a structured index, with a cutoff, with the numbers verified afterward and the result tied to a revision.
...
**Live Slack delivery** is proven. Card 1 (`finding_published` fnd-mtyrv4yj-y70h) posted at 19:22:30Z on September 12, 2026, 40 seconds after Juno's `@Rev` mention in `#ks4-electrical`: `read_thread`, 7 workspace searches, `read_evidence`, one RC run (2.208/2.435/6.4933 s), finding posted, then the section 4 proposal card. An earlier attempt at 19:03Z failed: the model's revision string was derived from a fixture seed message rather than the live `occurredAt`, so `publish_result` refused as stale and the 10-step budget was exhausted. Root cause diagnosed; fix committed in `21b6295` (`run_check` records the thread revision at run time; reviewer gets 24 steps). Pending the next Slack steps: Dara's 820 uF correction (Card 1 stale → Card 2) and Approve on the section 4 proposal.

**Live model inference** was also verified at 3:50 PM EDT before the live Slack attempt: `gpt-5.6-sol`, 45 s, 12 tool calls, Card 1 as scripted. Recorded: [`evals/records/model/scenario-a.1.json`](evals/records/model/scenario-a.1.json).

`npm run verify` passed on `main` as of 3:30 PM EDT. Test counts from STATUS.md at that point:
...
| `web` tests | 35 pass |
| `pet` (desktop overlay) tests | 53 pass |
...
Desktop companion, against the running web server:

```sh
npm run dev --workspace pet   # connects to http://127.0.0.1:3100 by default
```
...
OpenAI runs the reviewer agent: it decides which tools to call, in what order, and what to extract from the document and thread text. The model is explicitly not trusted with arithmetic — it reads values from text and passes them to a checker; `publish_result` refuses to post unless the numbers came from a checker run. In the live Slack run on September 12, `gpt-5.6-sol` made 12 tool calls and produced Card 1 as designed. OpenRouter can be substituted with any catalog model that supports tool use.

Key file: [`packages/agent-core/src/model.ts`](packages/agent-core/src/model.ts).

### Ambiguous AI

The web console's follow-up persistence uses Ambiguous AI when `AMBIGUOUS_API_KEY` is set. `/api/followups` calls `configuredWorkplace()` from [`apps/web/src/lib/server/workplace.ts`](apps/web/src/lib/server/workplace.ts), which connects to `https://app.ambiguous.ai/mcp` via the inherited workplace capability; without the key it falls back to local file storage at `.data/web-approvals`. The reviewer Slack agent explicitly opts out (`workplace: false` in `agent.ts`); Ambiguous does not affect the Slack card or the checker path. The channel and evidence graph are ThreadRev's work; the Ambiguous integration is inherited from the starter kit.
