...
- Record Option 1 (fresh thread). The channel process restarted at 3:33 PM when another lane saved edits under `apps/channel/src` (the dev script runs with `--watch`). On the managed path the SDK has a durable `IntelligenceStateStore`, so Card 1 and the proposal in the existing thread may still be known to the new process, but that is unverified. A fresh thread removes the question: seed message with the docx, firmware reply, @Rev mention, about 40 s to Card 1 and the proposal, then the correction, then Approve. Hold edits under `apps/channel/src` while recording; every save restarts the process mid-run.
- Recorder: Astra (the Codex node) on the desktop, Slack with the web console and the overlay on screen. Everything Astra needs is in this file; the live thread permalink above is the rehearsal reference, not the take.
